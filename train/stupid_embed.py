...
if __name__ == "__main__":
    ...
    start = 0
    end = 130_000# 1#1_000_000 #132121 -- is the total length
    # end = 2000
    ...
    random_projection = 1024#1#int(1024 / 2**i)
    print("Creating data for random_projection",random_projection)
    #random_projection = 512#I reduced because MoE was too large
    ...
    for i, conversation in zip(tqdm(range(start, end)), iter(dataset)):
        ...
        for j in range(len(conversation)):
            try:
                ...
                print("Failed likely due to context length exceeding GPU VRAM, to be dealt with separately")
                failed.append(i)
                break
        if len(e) == 0:
    ...
    dataset.save_to_disk(f"embeddings/lmsys-chat-1m_embeddings_{random_projection}_{end}")

    # Save failed indices to deal with separately (Run on GPU with more VRAM)
    np.save(f"embeddings/lmsys-chat-1m_embeddings_{random_projection}_{end}/failed.npy", np.array(failed, dtype=int))
